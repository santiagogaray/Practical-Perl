...
elsif ( $action eq "signin" ) {		# Sign-in form
    signin( $q, $id );
}
...
##############################################################
# Account sign-in. Displays form and sends values to
# load_account() function.
# If an id is returned its saved state is loaded
# Error message is diplayed when account not found. 
##############################################################
sub signin {
    my( $q, $id ) = @_;
    ...
    print header( $q, "Sign In" ), p("$message"),
          $q->start_form,
          $q->table(
              { -border       => 1,
                -cellspacing  => 1,
...
##############################################################
# Error message displayed if Sig-in not succesfull
##############################################################
sub display_error {
    my( $q, $id ) = @_;
    print header( $q, "No account" ), 
	  p("We did not find an account under that username,
	 	please use the link below to create a new account."),
          footer( $q, $id );
}
...
sub catalog {
    ...
    if ( $q->request_method eq "POST" ) {
	save_state( $q );
	$do_calc = 1;		# do calculations when update
    }
...
sub load_state {
    ...
    $saved or return 1;		# not successful load
    
    foreach ( $saved->param ) {
        $q->param( $_ => $saved->param($_) ) unless defined $q->param($_);
    }
    return 0;	# succesful load
}
...
sub get_items {
   	my $q = shift;
    	...
    	my $dbh = open_dbase( 'hw5', 'catalog', 'catalog' );
    		...
    		# Build a list of movie titles, quantities and prices
    	foreach ( $q->param ) {
...
##############################################################
# retrieves a saved account info from database and returns id
##############################################################
sub load_account {
    my( $q ) = @_;
	my $id = '';
...
sub save_account {
	...
	my $sqlh = exe_dbase( $dbh, qq{
		...
		( "$username", "$crpwd", "$email", "$creditcard", "$id" )
	} );
...
sub footer {
    ...
    my $signin_link = 
       $q->a( { -href => "$url?action=signin&id=$id" }, "Sign In" );
    my $account_link = 
    ...
    return $q->hr .
           $q->p( "[ $signin_link | $account_link | $catalog_link | $cart_link | $checkout_link ]" ) .
           $q->end_html;
}
